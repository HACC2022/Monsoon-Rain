...
export default class BillsController {
    ...
    public async getAllTestimonyForBillById({ request, response }: HttpContextContract) {
        try {
            ...
            return response.ok({ status: true, data: testimonies, message: 'Testimonies returned for a given bill id'})
        } catch (error) {
            console.log(error.message)
            return response.badRequest({ status : false, message : 'Could not find testimonies for given bill id'})
        }
    }
    ...
    public async postAssignUsers({ request, response }: HttpContextContract) {
        try {
            ...
            return response.ok({ status: true, data: updatedBill, message: 'Bill updated with new assigned users'})
        } catch (error) {
            console.log(error.message)
    ...
    public async postAssignOffices({ request, response }: HttpContextContract) {
        ...
        try {
            ...
            return response.ok({ status: true, data: updatedBill, message: 'Bill updated with new offices'})
        } catch (error) {
            console.log(error.message)
            return response.badRequest({ status : false, message : 'Could not update bill with new offices'})
        }
    }
    ...
    public async postAssignTestimony({ request, response }: HttpContextContract) {
        try {
            ...
            return response.ok({ status: true, data: updatedTestimony, message: 'Testimony updated with new body'})
        } catch (error) {
            console.log(error.message)
            return response.badRequest({ status : false, message : 'Could not update testimony with new body'})
        }
    }
}
